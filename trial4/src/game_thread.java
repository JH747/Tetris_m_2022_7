...
public class game_thread extends Thread{
    ...
    private void update_cBock_loc(){
        ...
        //ArrayList<uPoint> tmp_block_loc = new ArrayList<>(Tetris_m.cBlock_loc); //  새 리스트 만들어 대조함. 자기 자신과 겹치는 것은 상관 없도록
        for(uPoint p : Tetris_m.cBlock_loc){
            ...
            collapse_self = uPoint.doesHave(p.x, p.y+1, Tetris_m.cBlock_loc);
//            for(uPoint p2 : tmp_block_loc){
//                if(p2.x == p.x && p2.y == p.y+1) collapse_self = true;
//            }
            if(Tetris_m.status[p.y+1][p.x] != 0 && !collapse_self) can_proceed = false;
        }
        if(!can_proceed){
    ...
    private void update_shape_to_stat(){
        ...
        for(int i = 34; i >= 0; i--){
            for(int j = 0; j < 20; j++){
                //
                //if(!uPoint.doesHave(n,m,Tetris_m.cBlock_loc))
                // 바닥에 닿아 더이상 움직일 수 없는 경우
                if(Tetris_m.hit_floor_or_block) tmp_stat[m][n] = Tetris_m.status[i][j];
                else{
                    // 바닥에 닿지 않은 경우 current block을 제외하고 나머지만 처리한다.
                    // 이 경우 아랫 줄이 지워질 경우 current block도 아래로 내려와 에러발생을 방지하기 위해
                    if(!uPoint.doesHave(n,m,Tetris_m.cBlock_loc)) tmp_stat[m][n] = Tetris_m.status[i][j];
                }
                n++;
        ...
        // current block에 속하는 경우를 밖에서 처리한다. 그래야
